...
<main class="pt-90">
    ...
    <section class="product-single container">
      <div class="row">
        ...
        @else
          <span class="text-danger">Please supply a valid price.</span>
        @endif
            </span>
          </div>
          <div class="product-single__short-desc">
            <p>{{ $product && $product->description ? $product->description : 'Description Not Available' }}</p>
          </div>
          @if(Cart::instance('cart')->content()->where('id', $product->id)->count())
           <a href="{{ route('cart.index') }}" class="btn btn-outline-primary">Go to Cart</a>
          @else
    ...
    @endif
          ...
          <div class="product-single__meta-info">
            ...
            <div class="meta-item">
              ...
              <span>{{ $product && $product->category ? $product->category->name : 'Category Not Available' }}</span>
            </div>
            <div class="meta-item">
              <label>Tags:</label>
              <span>NA</span>
                  ...
                  <img loading="lazy" src="{{ asset('uploads/products') }}/{{ $product->image }}" width="330" height="400"
                    ...
                    @foreach(explode(',', $product->images) as $gimg)
                      <img loading="lazy" src="{{ asset('uploads/products') }}/{{ $gimg }}" width="330" height="400"
                        alt="{{ $product->name }}" class="pc__img pc__img-gallery">
                    @endforeach
                </a>
                @if(Cart::instance('cart')->content()->where('id', $product->id)->count())
            ...
            @csrf
             <input type="hidden" name="id" value="{{ $product->id }}">
              ...
              <div class="pc__info position-relative">
                <p class="pc__category">{{ $product->category ? $product->category->name : 'Category Not Available' }}</p>
