    ...
    <section class="shop-main container d-flex pt-4 pt-xl-5">
          ...
          <div class="accordion-item mb-4 pb-3">
            ...
            <div id="accordion-filter-1" class="accordion-collapse collapse show border-0"
              ...
              <div class="accordion-body px-0 pb-0 pt-3">
                ...
                <li class="list-item">
                    <span class="menu-link py-1 category_list">
                      <input type="checkbox" name="category" value="{{ $item->id }}" class="chk_category">
                      {{$item->name}}
                    </span>
                    <span class="text-right float-end">{{ $item->products_count }}</span>
                  </li>
                @endforeach
                </ul>
              </div>
            ...
            <div id="accordion-filter-brand" class="accordion-collapse collapse show border-0"
              ...
              <ul class="list list-inline mb-0">
                ...
                <span class="menu-link py-1">
                  @if($f_brands && in_array($item->id, explode(',', $f_brands)))
                    <input type="checkbox" name="brand" value="{{ $item->id }}" class="chk_brand" checked>
                  @else
                    <input type="checkbox" name="brand" value="{{ $item->id }}" class="chk_brand">
                  @endif
                  {{$item->name}}
                </span>
      ...
      <div class="shop-list flex-grow-1">
        ...
        <div class="d-flex justify-content-between mb-4 pb-md-2">
          ...
          <div class="shop-acs d-flex align-items-center justify-content-between justify-content-md-end flex-grow-1 styles="margin-right: 20px;">
          ...
          <div class="product-card-wrapper">
            <div class="product-card mb-3 mb-md-4 mb-xxl-5">
              <div class="pc__img-wrapper">
                      ...
                      @foreach(explode(',', $product->images) as $gimg)
                        <div class="swiper-slide">
                          <a href="{{ route('shop.product.detail', ['product_slug' => $product->slug]) }}">
                            <img loading="lazy" src="{{ asset('uploads/products') }}/{{ $gimg }}" width="330" height="400" alt="{{ $product && $product->name ? $product->name : '' }}" class="pc__img">
                          </a>
                        </div>
                      @endforeach
                    @endif
                ...
                @if(Cart::instance('cart')->content()->where('id', $product->id)->count()>0)
                   <a href="{{ route('cart.index') }}" class="pc__atc btn anime_appear-button btn position-absolute border-0 text-uppercase fw-medium  btn-outline-primary">Go to Cart</a>
                   @else
            ...
            <input type="hidden" name="quantity" value="1">
              ...
              <input type="hidden" name="price" value="
                @if($product && is_numeric($product->sale_price) && $product->sale_price > 0)
                  {{ $product->sale_price }}
                @elseif($product && is_numeric($product->regular_price) && $product->regular_price > 0)
        ...
        <div class="d-flex justify-content-center mt-4 pt-2 pt-md-3">
          {{ $products->withQueryString()->links('pagination::bootstrap-5') }}
        </div>
      </div>
    </section>
  </main>
  <form id="pagesize-form" method="GET" action="{{ route('shop.index') }}">
    @csrf
    <input type="hidden" name="size"  value="{{ $products->currentPage() }}">
    <input type="hidden" name="size" id="size" value="{{ $size }}" />
    <input type="hidden" name="order" id="order" value="{{ $order }}" />
  <input type="hidden" name="brand" id="brand" value="{{ $f_brands }}" />
...
$(function(){
  ...
  $(".chk_brand").on("change", function(){
    $("#brand").val($(".chk_brand:checked").map(function(){
      return $(this).val();
    }).get().join(","));
    $("#pagesize-form").submit();
  });
  $(".chk_category").on("change", function(){
    $("#category").val($(".chk_category:checked").map(function(){
      return $(this).val();
    }).get().join(","));
    $("#pagesize-form").submit();
  });
  $(".price-range-slider").on("change", function(){
    $("#min_price").val($(this).data("slider-value")[0]);
    $("#max_price").val($(this).data("slider-value")[1]);
    $("#pagesize-form").submit();
  });
